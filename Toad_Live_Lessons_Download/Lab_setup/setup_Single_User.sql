...
create table a as
select rownum b_id, 'OPEN' status
from all_objects
where rownum <= 100;
...
create index a_status_idx on a (status) compute statistics;
create index b_status_idx on b (status) compute statistics;
ALTER TABLE b ADD CONSTRAINT pk_b_id PRIMARY KEY (id);
create index c_b_id_idx on c (b_id) compute statistics;

commit;



rem ADV PL/SQL Setup
